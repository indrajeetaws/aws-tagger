...
@click.command()
@click.option('--tagcolumnheader', help='AWs Colume key value')
...
def cli(tagcolumnheader, dryrun, verbose, region, role, resource, tag, csv):
    if csv and (len(resource) > 0 or len(tag) > 0):
        print("Cannot use --resource or --tag with --csv option")
        sys.exit(1)
    if csv:
        tagger = CSVResourceTagger(tagcolumnheader, dryrun, verbose, role, region, tag_volumes=True)
        tagger.tag(csv)
    else:
