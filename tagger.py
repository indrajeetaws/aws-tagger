...
class SingleResourceTagger(object):
    def __init__(self, dryrun, verbose, role=None, region=None, tag_volumes=False):
        ...
        self.taggers['apigateway'] = APITagger(dryrun, verbose, role=role, region=region)
    ...
    def tag(self, resource_id, tags):
        ...
        if resource_id.startswith('arn:'):
            ...
            tagger = self.taggers['apigateway']
...
class APITagger(object):
    def __init__(self, dryrun, verbose, role=None, region=None):
        self.dryrun = dryrun
        self.verbose = verbose
        self.aapigateway = _client('apigateway', role=role, region=region)
    ...
    def tag(self, resource_arn, tags):
        ...
        if not self.dryrun:
            try:
                print("tagging %s with %s" % (resource_arn, tags))                          
                self._apigateway_tag_resource(resourceArn=resource_arn, tags=tags)
            except botocore.exceptions.ClientError as exception:
                if exception.response["Error"]["Code"] in ['ResourceNotFoundException']:
                    print("Resource not found: %s" % resource_arn)
                else:
                    print("Resoiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii")
                    raise exception

    @retry(retry_on_exception=_is_retryable_exception, stop_max_delay=30000, wait_exponential_multiplier=1000)
    def _apigateway_tag_resource(self, **kwargs):
        return self.aapigateway.tag_resource(**kwargs)
